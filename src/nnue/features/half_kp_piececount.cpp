...
namespace Eval::NNUE::Features {
  ...
  inline IndexType HalfKP_PieceCount<AssociatedKing>::MakeIndex(
      Color perspective, Square s, Piece pc, Square ksq, int pieceCount) {
    ...
    return IndexType(orient(perspective, s) + kpp_board_index[pc][perspective] + PS_END * ksq
           + pieceCount_index * static_cast<IndexType>(SQUARE_NB) * static_cast<IndexType>(PS_END));
  }
  ...
  void HalfKP_PieceCount<AssociatedKing>::AppendActiveIndices(
    ...
    while (bb) {
      Square s = pop_lsb(&bb);
      active->push_back(MakeIndex(perspective, s, pos.piece_on(s), ksq, pos.state()->allPiecesCount));
    }
  }
  ...
  void HalfKP_PieceCount<AssociatedKing>::AppendChangedIndices(
    ...
    for (int i = 0; i < dp.dirty_num; ++i) {
      ...
      if (dp.from[i] != SQ_NONE)
        removed->push_back(MakeIndex(perspective, dp.from[i], pc, ksq, pos.state()->previous->allPiecesCount));
      if (dp.to[i] != SQ_NONE)
        added->push_back(MakeIndex(perspective, dp.to[i], pc, ksq, pos.state()->allPiecesCount));
    }
  }
