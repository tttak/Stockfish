...
#ifndef _NNUE_FEATURES_HALF_KPKRANK_H_
#define _NNUE_FEATURES_HALF_KPKRANK_H_
...
// Feature HalfKPKrank: Combination of the position of own ball or enemy ball and the position of pieces other than balls
template <Side AssociatedKing>
class HalfKPKrank {
 public:
  // feature quantity name
  static constexpr const char* kName =
      (AssociatedKing == Side::kFriend) ? "HalfKPKrank(Friend)" : "HalfKPKrank(Enemy)";
  // Hash value embedded in the evaluation function file
  static constexpr std::uint32_t kHashValue =
      0x1D39D5B9u ^ (AssociatedKing == Side::kFriend);
  // number of feature dimensions
  static constexpr IndexType kDimensions =
      static_cast<IndexType>(SQUARE_NB) * static_cast<IndexType>(fe_end) * 8;
